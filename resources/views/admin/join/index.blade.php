            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    <div class="col-lg-6 col-7">
                        <h6 class="h2 text-white d-inline-block mb-0">我参与的项目列表</h6>
...
@section('js')
    ...
    <script>
            ...
            oTableInit.Init = function () {
                $('#table_id_example').bootstrapTable({
                    ...
                    columns:[
                        ...
                        {title:'押金缴纳时间', field:'created_at',},
                        {title:'保证金状态', field:'deposit_type',},
                        {title:'项目状态', field:'check',},
        ...
        $("#status").on("change", function() {
            $('#table_id_example').bootstrapTable('refresh', {url: '/admin/demand/join/list'});
        });
    </script>
    <script>
        function check(data) {
        ...
        function submit(){
            ...
            var url = "{{ route('demand.join.check') }}";
            $.ajax({
                type: 'post',
                url: url,
                data: form,
        ...
        function confirm_check(pro_id,pr_mer_id){
            $.ajax({
                type: 'post',
                url: "{{route('demand.join.download')}}",
                data: {'project_id':pro_id,'pro_mer_id':pr_mer_id},
                dataType: 'json',
                success: function(data){
                    Swal.fire({
                        title: '确认后退回项目押金！',
                        ...
                        if (result.value) {
                            var url = "{{ route('demand.join.confirm_check') }}";
                            $.ajax({
                                type: 'post',
                                url: url,
                                data: {'id':pro_id},
